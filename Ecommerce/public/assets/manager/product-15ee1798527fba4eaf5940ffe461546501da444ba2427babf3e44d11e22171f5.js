        ...
        $.ajax({
          ...
          success: function (data) {
            if (data.code == 200) {
              ...
              console.log(total_order_of_month + data.data.fee_ship)
              console.log(total_order_of_month +  parseFloat(data.data.fee_ship))

              if (data.data.carts_order.length > 0) {
                $(".total_order_of_month").css("display", "block");
                $(".total_order_of_month").get(0).innerText =
                  "TOTAL ORDER OF MONTH " + "$" + total_order_of_month + data.data.fee_ship + " (include fee ship)";
              } else {
