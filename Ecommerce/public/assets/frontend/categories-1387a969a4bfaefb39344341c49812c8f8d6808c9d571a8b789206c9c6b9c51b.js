      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.code == 200) {
            ...
            $(ele).find(".list-category").append(template_category({
      ...
      if (event.keyCode == 13) {
        ...
        if (check_login == 1) {
          $.ajax({
            ...
            type: 'POST',
            data: { comment: comment, product_id: product_id, token: module.settings.api.api_token },
            dataType: 'json',
            success: function(data) {
              if (data.code == 200) {
        ...
        if (check_login == 1) {
          if (check_rate == 0) {
            $.ajax({
              ...
              type: 'POST',
              data: { rate: rate, product_id: product_id, token: module.settings.api.api_token },
              dataType: 'json',
              success: function(data) {
                if (data.code == 200) {
                  $(".list-rate").html("");
                  var template_rate = Handlebars.compile(module.settings.template.rate.html());
                  $(ele).find(".list-rate").append(template_rate({
                    avg: data.data.avg,
                  }));
          ...
          alert ("ban da danh gia san pham nay!");
        }
        }
        else {
          alert ("Please to login before Rate!");
        }
    });
  }
    ...
    $(document).on("keypress", ".in-comment-edit", function (e) {
      ...
      if (event.keyCode == 13) {
        ...
        $.ajax({
          ...
          type: 'POST',
          data: { comment: comment, comment_id: comment_id, token: module.settings.api.api_token },
          dataType: 'json',
          success: function(data) {
            if (data.code == 200) {
              var template_edit_comment = Handlebars.compile(module.settings.template.edit_comment.html());
              $(ele).find(".list-edit-comment").html(template_edit_comment({
                comment: data.data.comment,
              }));
