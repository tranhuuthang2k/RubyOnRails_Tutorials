function Post(options) {
    ...
    module.clickPost = function(){
      ...
      module.clickNotification = function(){
        $(document).on('click','.notifications',function(){
          $.ajax({
            url: module.settings.api.notifications,
            headers: {
              'Api-Token': module.settings.api.token  //z9SNcLnCMHJUXdtzU0VBeQ
            },
            type: 'GET',
            ...
            success: function(data) {
              if (data.code == 200) {
                console.log('yeah')
                var template_notification = Handlebars.compile(module.settings.template.notifications.html());
                $("#list-notifications").html("");
                $("#list-notifications").append(template_notification({
